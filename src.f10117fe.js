...
},{}],"node_modules/rgbquant/src/rgbquant.js":[function(require,module,exports) {
/*
...
},{}],"src/comparator.ts":[function(require,module,exports) {
"use strict";
...
function getRandomColor(palette) {
  if (palette.length > 0) {
    var num = palette[Math.floor(Math.random() * palette.length)];
    return "rgb(" + num[0] + "," + num[1] + "," + num[2] + ")"; //console.log(num.toString(16));
...
var rgbquant_1 = __importDefault(require("rgbquant"));
...
function () {
  ...
  PainterApp.prototype.start = function () {
    ...
    ctx.drawImage(this.srcimg, 0, 0, this.srcimg.naturalWidth, this.srcimg.naturalHeight, 0, 0, this.image2.width, this.image2.height); // convert

    var self = this;
    setTimeout(function () {
      var pctx = self.image2.getContext("2d"); // now grab the downsampled pixels and hold onto them

      self.sourcePixels = pctx.getImageData(0, 0, _this.image2.width, _this.image2.height); // options with defaults (not required)
      ...
      var q = new rgbquant_1.default(opts); // analyze histograms

      q.sample(self.sourcePixels); // build palette

      self.palette = q.palette(true); //      self.palette = palette(self.sourcePixels.data, 256, 2);
  ...
  PainterApp.prototype.restartPainting = function () {
    ...
    this.animationId = 0; // clear the canvas
  ...
  PainterApp.prototype.setupGui = function () {
    ...
    this.gui.add(this.guiState, "image", imageChoices).name("Image").onChange(function (value) {
      _this.play = false; //clearTimeout(this.animationId);

      cancelAnimationFrame(_this.animationId);
      _this.animationId = 0; // initiate load of new image
  ...
  PainterApp.prototype.iterate = function () {
    ...
    if (newdiff < this.similarity) {
      ...
      destCtx.drawImage(this.imageTemp, 0, 0); //      destCtx.globalCompositeOperation = "source-over";

      this.numStrokesKept += 1;
    } // 4. else don't
    else {
        // copy image1 into temp image
        var destCtx = this.imageTemp.getContext("2d"); //destCtx.globalCompositeOperation = "copy";

        destCtx.drawImage(this.image1, 0, 0); //      destCtx.globalCompositeOperation = "source-over";
      }
...
},{"dat.gui":"node_modules/dat.gui/build/dat.gui.module.js","rgbquant":"node_modules/rgbquant/src/rgbquant.js","./comparator":"src/comparator.ts","./brush":"src/brush.ts"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
